...
export class AndroidQuiz extends Component {
  ...
  setAnswer = (response, page) => {
    ...
    this.setState(
      ...
      () => {
        const { correctQuestionCount, incorrectQuestionCount, totalQuestionCount } = this.state;

        if (totalQuestionCount === correctQuestionCount + incorrectQuestionCount) {
  ...
  render() {
    ...
    if (this.state.show === screen.RESULT) {
      ...
      const percent = ((correctQuestionCount / totalQuestionCount) * 100).toFixed(0);
      const resultStyle =
        percent >= 70 ? styles.goodResult : styles.badResult;
          ...
          <View style={styles.pageStyle} key={idx}>
            ...
            <View>
              ...
              <TouchButton
                ...
                onPress={() => this.setAnswer(answer.INCORRECTQUESTIONCOUNT, idx)}
                disabled={this.state.answered[idx] === 1}
              >
                Incorrect
              </TouchButton>
